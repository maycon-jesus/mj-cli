...
func newBranchNewCommand() *commands.Command {
	return &commands.Command{
		...
		Translations: intl.Translations{
			"en": {
				"command.git.new.short_description":   "Checkout to main/master and create a new branch",
			...
			"pt-BR": {
				"command.git.new.short_description":   "Faz checkout em main/master e cria uma nova branch",
		...
		Handler: func(ctx context.Context, execData *commands.ExecData) error {
			...
			spinner.Start()
			err = cmd.RunCommand(fmt.Sprintf("git checkout %s", mainBranch))
			if err != nil {
			...
			spinner.Start()
			err = cmd.RunCommand(fmt.Sprintf("git checkout -b %s", branchName))
			if err != nil {
